...
If your database predates the men-or-women-only change, re-run `schema.sql` and
then `seed.sql`. The first narrows the `gender` check and moves any `mixed` rows
to `female`; the second rewrites the demo listings. Until both have run, old
listings show the word "mixed" where the label should be.
